...
In addition to evaluating expressions we can also ask for their type with the `:type` (abbreviated `:t`) GHCi command. The type of `"asdf"` is a list of characters. Note that in Haskell, a `String` is defined as a list of characters `[Char]`, this can also show up as:
